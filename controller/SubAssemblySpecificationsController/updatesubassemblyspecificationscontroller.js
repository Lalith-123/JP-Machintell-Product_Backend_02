...
const updatesubassemblyspecificationsController = async(req, res) => {
    try
    {
        ...
        const result = await updatesubassemblyspecificationsModel( sub_assembly_specs,sub_assembly_unit,sub_assembly_value, id);
        res.status(200).json({message: 'sub_assemblies_specifications Updated Successfully', data: result});
    }
    catch (err)
    {
        console.log(err);
